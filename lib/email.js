...
import { buildSeanceInvoicePdf, seanceInvoiceFilename } from './invoice-pdf.js';
...
export function confirmationSubject({ amiPrenom, hostPrenom } = {}) {
  if (hostPrenom) return `${hostPrenom} t’a inscrit(e) à une séance d’essai offerte — Boxing Center`;
  if (amiPrenom) return `Ta séance d’essai est confirmée — avec ${amiPrenom}`;
...
function confirmationHeadline({ prenom, jourNom, amiPrenom, hostPrenom }) {
  ...
  if (hostPrenom) {
    return jour ? `${hostPrenom} t’a inscrit(e) — à ${jour}.` : `${hostPrenom} t’a inscrit(e).`;
  }
...
function confirmationLead({ prenom, salleLabel, amiPrenom, hostPrenom }) {
  ...
  if (hostPrenom) {
    return `${prenom}, ${hostPrenom} vient de t’offrir une séance d’essai${salle}. Tu n’as rien à régler. Vous n’êtes pas obligés de venir le même jour : ta séance reste offerte, même si tu passes un autre moment.`;
  }
  if (amiPrenom) {
    return `${prenom}, ta séance d’essai est confirmée${salle}, et celle de ${amiPrenom} aussi. ${amiPrenom} reçoit son propre mail. Vous n’êtes pas obligés d’arriver ensemble : sa séance reste offerte.`;
  }
  return `Ta séance d’essai est enregistrée${salle}. Ta facture (0 €) est jointe à cet email.`;
}
...
export function confirmationText(input = {}) {
  const { prenom, salleLabel, jourLabel, hostPrenom, amiPrenom } = input;
  if (hostPrenom) {
    return [
      `Bonjour ${prenom},`,
      '',
      ...
      '',
      `Inscrit(e) par : ${hostPrenom}`,
      `Salle : ${salleLabel}`,
      `Jour prévu : ${jourLabel}`,
      'À régler : 0 €',
      '',
      'Présente-toi à l’accueil en tenue de sport. Le matériel est prêté. Une bouteille d’eau suffit. Vous n’êtes pas obligés de venir le même jour : ta séance reste offerte.',
      '',
  ...
  return [
    ...
    `Salle : ${salleLabel}`,
    `Jour prévu : ${jourLabel}`,
    amiPrenom ? `Accompagné : oui — ${prenom} et ${amiPrenom}` : 'Accompagné : non',
    '',
    'À régler sur place : 0 € — au lieu de 10 €.',
    '',
    amiPrenom
      ? 'Présente-toi à l’accueil en tenue de sport. Le matériel est prêté. Une bouteille d’eau suffit. Vous n’êtes pas obligés d’arriver ensemble : sa séance reste offerte, même s’il passe un autre jour.'
      : 'Présente-toi à l’accueil en tenue de sport. Le matériel est prêté. Une bouteille d’eau suffit. Ta facture (0 €) est jointe à cet email.',
    '',
    'L’équipe Boxing Center',
  ].join('\n');
}
...
export function confirmationHtml(input = {}) {
  ...
  const guest = Boolean(input.hostPrenom);
  const kicker = guest ? 'Invitation — séance offerte' : 'Confirmation';
  const recap = guest
    ? `${row('Inscrit(e) par', escapeHtml(input.hostPrenom))}
                ${row('Salle', salle)}
                ${row('Jour prévu', jour)}
                ${row('À régler', '<span style="color:#e8001c;font-weight:700;">0 €</span> <span style="color:#9aa3b5;font-size:13px;">— offert par Boxing Center</span>')}`
    : `${row('Salle', salle)}
                ${row('Jour prévu', jour)}
                ...
                ${row(
                  'Accompagné',
                  input.amiPrenom
                    ? escapeHtml(`${input.prenom} et ${input.amiPrenom}`)
                    : 'Non'
                )}`;
        ...
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="width:100%;max-width:600px;background:#0c1018;border:1px solid #2a3140;">
          ...
          <tr>
            <td style="padding:28px 32px 12px;font-family:Arial,Helvetica,sans-serif;">
              ...
              <p style="margin:8px 0 0;font-size:12px;letter-spacing:0.16em;text-transform:uppercase;color:#9aa3b5;">${kicker}</p>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="padding:0 32px 28px;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.6;color:#c8cdd8;">
              ${guest ? 'Tu n’es pas obligé(e) de venir le même jour que la personne qui t’a inscrit(e) : ta séance reste offerte. L’équipe t’attend.' : 'Présente-toi à l\'accueil. Ta facture (0 €) est jointe à cet email. L\'équipe t\'attend.'}
            </td>
          </tr>
          <tr>
...
export function relanceProspectText({ prenom }) {
  return [
    `Bonjour ${prenom},`,
    '',
    'Vous avez fait l’essai chez Boxing Center — merci d’être venu(e).',
    'Vous aimeriez nos offres promo ? 29,99 € / 4 semaines sans engagement, ou 259 € / 12 mois.',
    '',
...
export function internalRecapHtml({ data, orderId, dryRun, error }) {
    const ami = data.ami
      ...
      : data.ami_pending
        ? '<p>Accompagnateur : pas encore répondu.</p>'
        : '<p>Pas d’ami(e).</p>';
  return `
...
export async function sendEmailViaBrevo({
  ...
  if (files.length) {
    payload.attachment = files.map((a) => ({
      name: a.name || a.filename || 'facture.pdf',
...
export async function sendConfirmationEmails(data, { dryRun = false, fetchImpl = fetch, onlyAmi = false } = {}) {
  ...
  const amiPayload = data.ami ? confirmationPayloadFromData(data, { asFriend: true }) : null;
  const friendEmail = data.ami?.email || '';
  const sendFriend = Boolean(friendEmail && !sameEmail(friendEmail, data.email));

  if (dryRun) {
    return {
      sent: false,
      reason: 'dry_run',
      preview: confirmationText(onlyAmi && amiPayload ? amiPayload : principal),
      would_send: [
        !onlyAmi && data.email ? { to: data.email, role: 'host', subject: confirmationSubject(principal) } : null,
        sendFriend ? { to: friendEmail, role: 'guest', subject: confirmationSubject(amiPayload) } : null,
      ].filter(Boolean),
    };
  }

  const results = [];
  const invoicePdf = buildSeanceInvoicePdf({
    orderId: data.orderId || data.order_id,
    prenom: data.prenom,
    nom: data.nom,
    email: data.email,
    salleLabel: data.gym?.label || data.salle_label || data.salle,
    visitLabel: [data.jour_nom, data.visit_date ? formatFrDate(data.visit_date) : '']
      .filter(Boolean)
      .join(' — '),
  });
  const invoiceAtt = {
    name: seanceInvoiceFilename(data.orderId || data.order_id),
    content: invoicePdf,
  };
  ...
  if (!onlyAmi) {
    results.push(
      await sendEmailViaBrevo({
        ...
        attachments: [invoiceAtt],
        fetchImpl,
      })
    );
  }
  if (sendFriend && amiPayload) {
    results.push(
      await sendEmailViaBrevo({
        to: friendEmail,
        subject: confirmationSubject(amiPayload),
        text: confirmationText(amiPayload),
        html: confirmationHtml(amiPayload),
        fetchImpl,
      })
    );
  } else if (friendEmail && !sendFriend) {
    results.push({ sent: false, reason: 'same_email_as_host', to: friendEmail });
  }
  return results;
}
...
<body style="margin:0;padding:0;background:#07090d;">
    ...
    <tr><td align="center">
      <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="width:100%;max-width:600px;background:#0c1018;border:1px solid #2a3140;">
        ...
        <tr><td style="padding:28px 32px 8px;font-family:Arial,Helvetica,sans-serif;">
          <p style="margin:0;font-size:11px;letter-spacing:0.22em;text-transform:uppercase;color:#e8001c;">Boxing Center</p>
          <h1 style="margin:12px 0 0;font-family:Georgia,Times,serif;font-size:28px;font-weight:400;color:#f4f1ea;line-height:1.2;">Vous avez fait l’essai</h1>
        </td></tr>
        <tr><td style="padding:16px 32px 8px;font-family:Arial,Helvetica,sans-serif;font-size:16px;line-height:1.6;color:#c8cdd8;">
          ...
          <p style="margin:0;">Vous avez fait l’essai chez Boxing Center. Vous aimeriez nos offres promo ? 29,99 € / 4 semaines, ou 259 € / 12 mois.</p>
        </td></tr>
  ...
  return sendEmailViaBrevo({
    to: lead.email,
    subject: 'Boxing Center — vous avez fait l’essai, découvrez nos offres promo',
    text,
    html,
    fetchImpl,
  });
