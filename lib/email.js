...
function confirmationLead({ prenom, salleLabel, amiPrenom, hostPrenom }) {
  ...
  if (hostPrenom) {
    return `${prenom}, ce n’est pas un mail de confirmation pour ${hostPrenom} : c’est le tien. ${hostPrenom} vient de t’inscrire à une séance d’essai offerte${salle}. Tu n’as rien à régler. Présentez-vous ensemble à l’accueil.`;
  }
  if (amiPrenom) {
    return `${prenom}, ta séance d’essai est confirmée${salle}, avec ${amiPrenom}. ${amiPrenom} reçoit un mail d’invitation de son côté — pas le même que celui-ci.`;
  }
...
export function confirmationText(input = {}) {
  ...
  if (hostPrenom) {
    return [
      ...
      'À régler : 0 €',
      '',
      'Présentez-vous ensemble à l’accueil, en tenue de sport. Le matériel est prêté.',
      '',
        ...
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="width:100%;max-width:600px;background:#0c1018;border:1px solid #2a3140;">
          ...
          <tr>
            <td style="padding:0 32px 28px;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.6;color:#c8cdd8;">
              ${guest ? 'Présentez-vous ensemble à l\'accueil. L\'équipe vous attend.' : 'Présente-toi à l\'accueil. L\'équipe t\'attend.'}
            </td>
          </tr>
          <tr>
